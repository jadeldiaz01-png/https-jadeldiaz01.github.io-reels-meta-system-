...
class PostgresBundleStore:
    def __init__(self, *, engine: AsyncEngine, registry: PostgresSkillRegistry, signer: OpenBaoTransitSigner, evidence: PostgresEvidenceLedger) -> None:
        self.engine = engine
    ...
    async def register_verified(self, *, name: str, version: str, attestation: BundleAttestation) -> None:
        ...
        if record.stage in {SkillStage.VALIDATED, SkillStage.PRODUCTION}:
            raise PermissionError("bundle_locked_after_validation")
        ...
        if not await self.signer.verify_digest(attestation.bundle_digest, attestation.signature, key_name=attestation.signer_key):
        ...
        await self.evidence.append(skill_name=name, version=version, event_type="bundle_verified", payload={
            "bundle_digest": attestation.bundle_digest,
            "manifest_digest": attestation.manifest_digest,
            "signer_key": attestation.signer_key,
        })
